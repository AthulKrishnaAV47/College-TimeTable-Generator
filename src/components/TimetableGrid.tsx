...
export default function TimetableGrid({
  ...
  return (
    <div className={`overflow-hidden rounded-xl border border-slate-200 bg-white ${className}`}>
      <div className="grid" style={{ gridTemplateColumns: "56px repeat(6, minmax(96px, 1fr))" }}>
        {/* Header row */}
        <div className="border-b border-slate-200 bg-slate-50" />
        {GRID_DAYS.map((d) => (
          <div
            key={d}
            className="border-b border-l border-slate-200 bg-slate-800 py-1.5 text-center text-xs font-semibold tracking-wide text-white uppercase"
          >
            {d.slice(0, 3)}
          </div>
        ))}
              ...
              {dayBlocks.map((b, i) => {
                ...
                return (
                  <div
                    ...
                    className="absolute inset-x-0.5 overflow-hidden rounded-md border px-1.5 py-0.5"
                    style={{
                      top: top + 1,
                      height: height - 2,
                      background: color.bg,
                  ...
                  >
                    <div className="truncate text-[10.5px] leading-tight font-bold">
                      {b.courseCode} · {b.slotCode}
                    </div>
                    {showName && (
                      <div className="truncate text-[10px] leading-tight">{b.courseName}</div>
                    )}
                    {showFaculty && (
                      <div
